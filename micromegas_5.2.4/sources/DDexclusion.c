...
static double DSllExclude(double pval, double *binSignal)
{
  ...
  double fbest=DSll(c2,binSignal);
//printf("c-best=%e fbest=%E\n", c2,fbest);  
  double fx= pval2plr(pval)*fbest;  

  if(c2<0 && fx>=1) return NAN; // no positive  solution 
  
      
  if(c2<=0) { c1=0;f1=1; c2=1;f2=DSll(c2,binSignal);} else { c1=c2; f1=f2; c2=c1*2;f2=DSll(c2,binSignal);}
...
static double DD_pval_(double s, char**expName)
{
  ...
  { 
     ...
     { double nEv=900*279*simpson_arg((funcArg)dNdERecoil,dNdE, XeEmin,E2,1E-3,NULL); 
       ...
       {  double Eev=13, dNdEev=5.22E-7,C;
          C=0.7*dNdERecoil(Eev,dNdE)/Xe1TpEff1(Eev)/dNdEev;
          pval_=exp(-s*nEv)*(1+s*C);  
       }   
